...
EG_CLASS_DECL( DeCreditsData )


void DeCreditsData::OnPropChanged( const egRflEditor& ChangedProperty , egRflEditor& RootEditor , eg_bool& bNeedsRebuildOut )
{
...
void DeCreditsData::PostLoad( eg_cpstr16 Filename, eg_bool bForEditor, egRflEditor& RflEditor )
{
...
void DeCreditsData::RefreshVisibleProperties( egRflEditor& RootEditor )
{
	...
	if( LinesEd )
	{
		for( eg_size_t i=0; i<LinesEd->GetNumChildren(); i++ )
		{
			...
			if( LineEd )
			{
				...
				if( TypeEd )
				{
					de_credits_line_t LineType = *reinterpret_cast<de_credits_line_t*>(TypeEd->GetData());
					switch( LineType )
					{
					case de_credits_line_t::Normal:
					...
					case de_credits_line_t::FutureHeaderPop:
						...
						SetChildEditable( "DelayTime" , false );
						break;
					case de_credits_line_t::Header:
						SetChildEditable( "Title" , true );
						SetChildEditable( "Name" , false );
						SetChildEditable( "DelayTime" , false );
						break;
					case de_credits_line_t::Center:
